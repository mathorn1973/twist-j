#!/usr/bin/env python3
"""Exact audit for P-PURE-QUBIT-RELATIONAL-GEOMETRY-2.
...
def audit_cauchy_binet():
    ...
    for n in (2, 3, 4):
        for entries in product(values, repeat=2 * n):
            ...
            minor_mass = sum((z.abs2() for z in minors), F(0))
            wedge_tensor_norm2 = 2 * minor_mass
            if gram_det != minor_mass or 2 * gram_det != wedge_tensor_norm2:
                return False, count
            if (minor_mass == 0) != all(z == ZERO for z in minors):
                return False, count
            count += 1
    return True, count
...
def main():
    print("P-PURE-QUBIT-RELATIONAL-GEOMETRY-2 verifier")
    ...
    U = ((I, ZERO), (ZERO, ONE))
    V = ((ONE, ZERO), (ZERO, ONE))
    A1 = ((HALF, HALF), (HALF, -HALF))
    A2 = apply_local(U, A1, V)
    d1, n1 = delta_norm(A1)
    d2, n2 = delta_norm(A2)
    _, tr1, _ = rho_data(A1)
    _, tr2, _ = rho_data(A2)
    gate("B1.local-U2.phase-breaker.normalized.modulus-invariant",
         tr1 == tr2 == 1
         and d1 == G(F(-1, 2)) and d2 == G(F(0), F(-1, 2))
         and n1 == n2 == F(1, 4))
    ...
    gate("R1.2xn.Cauchy-Binet.unhalved-factor.gaussian-grids", cb_ok)
    ...
    gate("R3.endpoints.product.4.maximally-entangled.8",
         4 * (1 + 4 * F(0) * F(1)) == 4
    ...
    gate("B2.Werner.p=1/2.PT-negative.CHSH-subcritical",
         pt_ok and horodecki_M == F(1, 2) < 1)
    gate("B2.Werner.pure-Pythagoras-fails",
         concurrence == F(1, 4) and concurrence * concurrence == F(1, 16) != 1)
    gate("B2.Werner.pure-CHSH-equation-fails",
         4 * horodecki_M - 4 != 4 * concurrence * concurrence)
    ...
    gate("B3.higher-rank.same-e2.different-spectrum",

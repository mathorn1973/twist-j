...
def _spawn(command: Sequence[str], *, stdin: int | BinaryIO) -> subprocess.Popen[bytes]:
    ...
    if os.name == "posix":
        ...
        keyword["creationflags"] = subprocess.CREATE_NEW_PROCESS_GROUP
    try:
...
def supervise_pipeline(
    ...
    *,
    ...
    try:
        ...
        assert (
            reader.stdin is not None
            and reader.stdout is not None
            and reader.stderr is not None
        )
...
def supervise_batch() -> BatchCapture:
    ...
    try:
        ...
        # This path is also bounded and fail-closed if an assertion above

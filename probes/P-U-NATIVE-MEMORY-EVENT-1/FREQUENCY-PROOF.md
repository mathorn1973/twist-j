# P-U-NATIVE-MEMORY-EVENT-1: fixed-reader frequency classification

**Prospective, result-exposed, proof-first, L1 only.** Issue #862.
The equations and proofs below were obtained in the disclosed incubation
C-U-NATIVE-MEMORY-EVENT-N (#861); this fresh formal probe does not claim blind
prediction or independent discovery. Its own preregistration and accepted
verifier must be committed and pushed before the formal gate execution.
These are proposed mathematical conclusions, not earned public claims;
review and any later Canon fold remain separate. This classifies the
preregistered mathematical reader class and adopts no physical occurrence
law. The formal verifier audits the finite certificate, while its own
EXPECTED.txt and RUN.md record the resulting execution provenance.
...
| Absolute LOW density with either bit restriction | `{k/60: 0<=k<=30}` | `{k/30: 0<=k<=15}` |

...
def g4_energy():
    ...
    for word, hs in k1_histories(12):
        ...
        # global sum of the localized source is the same invariant
...
def g5_g6_noether_coefficients():
    ...
    # Frozen linear system: alpha-beta=0, alpha-2 gamma=0, alpha=1.
    A = [[F(1), F(-1), F(0)],
         [F(1), F(0), F(-2)],
         [F(1), F(0), F(0)]]
    b = [F(0), F(0), F(1)]
...
def g12_static():
    # Workflow policy/security remains an external PR gate. This verifier-side
    # block proves only that the scientific script is deterministic and uses no
    # external input, randomness, floating-point library, network or subprocess.

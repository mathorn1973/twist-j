# P-PURE-QUBIT-RELATIONAL-GEOMETRY-1 preregistration
...
```text
probe:               P-PURE-QUBIT-RELATIONAL-GEOMETRY-1
public claim lock:   issue #428
probe owner:         A. M. Thorn / delegated session
branch:              probe/P-PURE-QUBIT-RELATIONAL-GEOMETRY-1
path:                probes/P-PURE-QUBIT-RELATIONAL-GEOMETRY-1/
initial base:        64d92817e2f63912d20240e92c780e2ec389c526
...
At lock time the four candidate IDs, the probe path and the branch are absent
from public `main`. Issues #419 and #422 are non-canonical linguistic and
mathematical lineage only. Their scratch/symbolic runs are not evidence for
this probe. The merged probe-only results in PRs #426 and #427 explicitly
exclude qubits, concurrence and CHSH; they neither imply nor earn these rows.
...
For R3 the external criterion is a named premise; the specialization to a
pure Schmidt state is derived below. No external software or dataset is used.
...
For a normalized pure `2 x n` vector this determinant fixes the two nonzero
Schmidt coefficients, but no standard two-qubit CHSH statement is claimed for
`n != 2`.
...
`2 x n` formula is the exact Cauchy-Binet identity for the two-row matrix.
...
Proof of R3 conditional on the named external criterion. In Schmidt gauge
the standard correlation tensor is
...
The Horodecki criterion gives `B_max=2 sqrt(m1+m2)` for the two largest
eigenvalues. Since `0<=C<=1`, `m1+m2=1+C^2`. Substitute `C=2||r||`.

#### 1.3 Frozen scope breakers

`B1`, local phase. For `A=I` and local unitaries `U=diag(i,1)`, `V=I`,
...
is `M=2(1/2)^2=1/2<1`, so it is CHSH-subcritical. Therefore neither R2 nor
the pure R3 relation is a mixed-state law.
...
S5  the external Horodecki criterion is a premise, not a TWIST-J derivation;

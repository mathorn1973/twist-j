...
from exact_matrix import (
    add, basis, cols, eye, flatten, independent, inv, mm, mpow, mv,
    neg, scale, sharp, sub, sum_mats, zero,
)
...
def independent(matrices):
    if not matrices:
        return True
    vectors = tuple(flatten(a) for a in matrices)
    return rank(cols(vectors)) == len(matrices)

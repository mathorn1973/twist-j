...
// representable, and otherwise uses the self-contained ExactUInt backend for
// both the powers of two and unbiased bounded rejection.
...
// Unsigned exact arithmetic with a no-allocation uint64 fast path and a
// canonical little-endian base-2^32 spill representation.  Dynamic storage is
// used only for values above UINT64_MAX, which keeps the legacy audit envelope
// cheap while allowing orbit weights of any practically addressable width.
class ExactUInt {
  public:
    ExactUInt() = default;
    ExactUInt(std::uint64_t value) : small_(value) {}

    bool is_zero() const { return limbs_.empty() && small_ == 0; }
    bool uses_dynamic_storage() const { return !limbs_.empty(); }

    std::size_t bit_length() const {
        if (limbs_.empty()) return bit_length_u64(small_);
        return (limbs_.size() - 1) * 32 + bit_length_u32(limbs_.back());
    }

    bool fits_u64() const { return limbs_.empty(); }

    std::uint64_t to_u64() const {
        if (!fits_u64()) throw IntegrityError("exact_uint_does_not_fit_u64");
        return small_;
    }

    std::string to_decimal() const {
        if (limbs_.empty()) return std::to_string(small_);
        constexpr std::uint64_t DECIMAL_BASE = 1000000000ULL;
        ...
        while (!work.empty()) {
            ...
            for (std::size_t index = work.size(); index-- > 0;) {
                const std::uint64_t current =
                    (remainder << 32) | static_cast<std::uint64_t>(work[index]);
                work[index] = static_cast<std::uint32_t>(current / DECIMAL_BASE);
    ...
    ExactUInt &operator+=(const ExactUInt &other) {
        if (limbs_.empty() && other.limbs_.empty()) {
            if (other.small_ <= std::numeric_limits<std::uint64_t>::max() - small_) {
                small_ += other.small_;
                return *this;
            }
        }
        const std::size_t count = std::max(limb_count(), other.limb_count());
        ensure_limb_capacity(count + 1);
        std::vector<std::uint32_t> result(count + 1, 0);
        std::uint64_t carry = 0;
        for (std::size_t index = 0; index < count; ++index) {
            const std::uint64_t sum = static_cast<std::uint64_t>(limb(index))
                + static_cast<std::uint64_t>(other.limb(index)) + carry;
            result[index] = static_cast<std::uint32_t>(sum);
            carry = sum >> 32;
        }
        result[count] = static_cast<std::uint32_t>(carry);
        assign_normalized(std::move(result));
        return *this;
    }

    ExactUInt &operator-=(std::uint64_t value) {
        if (limbs_.empty()) {
            if (small_ < value) throw IntegrityError("exact_uint_subtraction_underflow");
            small_ -= value;
            return *this;
        }
        ...
        while (remaining != 0 || borrow != 0) {
            if (index == limbs_.size()) {
                throw IntegrityError("exact_uint_subtraction_underflow");
            }
            const std::uint64_t subtrahend = (remaining & LIMB_MASK) + borrow;
            const std::uint64_t current = limbs_[index];
            limbs_[index] = static_cast<std::uint32_t>(
                (LIMB_BASE + current - subtrahend) & LIMB_MASK
            );
    ...
    ExactUInt &operator<<=(std::uint64_t shift) {
        if (shift == 0 || is_zero()) return *this;
        if (shift > std::numeric_limits<std::size_t>::max()) {
            throw IntegrityError("exact_uint_shift_too_large");
        }
        const std::size_t amount = static_cast<std::size_t>(shift);
        const std::size_t old_bits = bit_length();
        if (amount > std::numeric_limits<std::size_t>::max() - old_bits) {
            throw IntegrityError("exact_uint_shift_too_large");
        }
        const std::size_t new_bits = old_bits + amount;
        if (limbs_.empty() && new_bits <= 64) {
            small_ <<= static_cast<unsigned>(amount);
            return *this;
        }
        const std::size_t new_count = (new_bits - 1) / 32 + 1;
        ensure_limb_capacity(new_count);
        std::vector<std::uint32_t> result(new_count, 0);
        const std::size_t word_shift = amount / 32;
        const unsigned bit_shift = static_cast<unsigned>(amount % 32);
        const std::size_t old_count = limb_count();
        for (std::size_t index = 0; index < old_count; ++index) {
            const std::uint64_t shifted =
                static_cast<std::uint64_t>(limb(index)) << bit_shift;
            result[index + word_shift] |= static_cast<std::uint32_t>(shifted);
            if (bit_shift != 0 && index + word_shift + 1 < result.size()) {
                result[index + word_shift + 1] |= static_cast<std::uint32_t>(shifted >> 32);
            }
        }
        assign_normalized(std::move(result));
        return *this;
    ...
    friend ExactUInt operator+(ExactUInt left, const ExactUInt &right) {
        left += right;
        return left;
    }

    friend ExactUInt operator-(ExactUInt left, std::uint64_t right) {
        left -= right;
        return left;
    }

    friend ExactUInt operator*(const ExactUInt &left, const ExactUInt &right) {
        if (left.is_zero() || right.is_zero()) return ExactUInt(0);
        if (left.limbs_.empty() && right.limbs_.empty()
            && right.small_ <= std::numeric_limits<std::uint64_t>::max() / left.small_) {
            return ExactUInt(left.small_ * right.small_);
        }
        const std::size_t left_count = left.limb_count();
        const std::size_t right_count = right.limb_count();
        if (right_count > std::numeric_limits<std::size_t>::max() - left_count) {
            throw IntegrityError("exact_uint_product_too_large");
        }
        const std::size_t result_count = left_count + right_count;
        ensure_limb_capacity(result_count);
        std::vector<std::uint32_t> result(result_count, 0);
        for (std::size_t i = 0; i < left_count; ++i) {
            ...
            for (std::size_t j = 0; j < right_count; ++j) {
                const std::uint64_t product =
                    ...
                    + result[i + j] + carry;
                result[i + j] = static_cast<std::uint32_t>(product);
                carry = product >> 32;
            }
            std::size_t index = i + right_count;
            while (carry != 0) {
                if (index == result.size()) {
                    throw IntegrityError("exact_uint_product_carry_overflow");
                }
                const std::uint64_t sum =
                    static_cast<std::uint64_t>(result[index]) + carry;
                result[index] = static_cast<std::uint32_t>(sum);
                carry = sum >> 32;
                ++index;
            }
        ...
        ExactUInt product;
        product.assign_normalized(std::move(result));
        return product;
    }

    friend bool operator==(const ExactUInt &left, const ExactUInt &right) {
        if (left.limbs_.empty() || right.limbs_.empty()) {
            return left.limbs_.empty() && right.limbs_.empty()
                && left.small_ == right.small_;
        }
    ...
    friend bool operator<(const ExactUInt &left, const ExactUInt &right) {
        if (left.limbs_.empty() || right.limbs_.empty()) {
            if (left.limbs_.empty() && right.limbs_.empty()) {
                return left.small_ < right.small_;
            }
            return left.limbs_.empty();
        }
  ...
  private:
    ...
    static constexpr std::uint64_t LIMB_MASK = LIMB_BASE - 1;

    static std::size_t bit_length_u32(std::uint32_t value) {
        std::size_t result = 0;
        while (value != 0) {
            ++result;
            value >>= 1;
    ...
    static std::size_t bit_length_u64(std::uint64_t value) {
        std::size_t result = 0;
        while (value != 0) {
            ++result;
            value >>= 1;
    ...
    std::size_t limb_count() const {
        ...
        return small_ <= LIMB_MASK ? 1 : 2;
    }

    std::uint32_t limb(std::size_t index) const {
        if (!limbs_.empty()) return index < limbs_.size() ? limbs_[index] : 0;
        if (index == 0) return static_cast<std::uint32_t>(small_);
        if (index == 1) return static_cast<std::uint32_t>(small_ >> 32);
        return 0;
    }

    static void ensure_limb_capacity(std::size_t count) {
        const std::vector<std::uint32_t> empty;
        if (count > empty.max_size()) throw IntegrityError("exact_uint_allocation_too_large");
    }

    void assign_normalized(std::vector<std::uint32_t> limbs) {
    ...
    void normalize() {
        ...
        if (limbs_.size() <= 2) {
            small_ = limbs_.empty() ? 0 : limbs_[0];
            if (limbs_.size() == 2) {
...
static_assert(
    ...
    "qualification requires a 64-bit long long"
);

std::uint64_t ordered_exponent_key(long long value) {
    return static_cast<std::uint64_t>(value) ^ (std::uint64_t{1} << 63);
}

std::uint64_t exponent_distance(long long high, long long low) {
    if (high < low) throw IntegrityError("orbit_exponent_order_invalid");
    return ordered_exponent_key(high) - ordered_exponent_key(low);
}

// Backend KATs deliberately emit no transcript line.  In particular, these
// force and inspect real dynamic storage rather than merely exercising values
// that happen to fit the inline representation.
void audit_exact_uint_backend() {
    const ExactUInt zero;
    require(zero.is_zero(), "exact_uint_zero_failed");
    require(zero.bit_length() == 0, "exact_uint_zero_bit_length_failed");
    require(zero.to_decimal() == "0", "exact_uint_zero_decimal_failed");
    require(!zero.uses_dynamic_storage(), "exact_uint_zero_spilled");

    const ExactUInt inline_max(std::numeric_limits<std::uint64_t>::max());
    require(inline_max.bit_length() == 64, "exact_uint_inline_bit_length_failed");
    require(!inline_max.uses_dynamic_storage(), "exact_uint_inline_max_spilled");
    require(
        inline_max.to_decimal() == "18446744073709551615",
        "exact_uint_inline_decimal_failed"
    );

    ExactUInt two_to_64 = inline_max;
    two_to_64 += 1;
    require(two_to_64.uses_dynamic_storage(), "exact_uint_add_did_not_spill");
    require(two_to_64.bit_length() == 65, "exact_uint_spill_bit_length_failed");
    require(
        two_to_64.to_decimal() == "18446744073709551616",
        "exact_uint_spill_decimal_failed"
    );
    ExactUInt shifted_two_to_64 = 1;
    shifted_two_to_64 <<= 64;
    require(shifted_two_to_64 == two_to_64, "exact_uint_shift_64_failed");

    ExactUInt demoted = two_to_64;
    demoted -= 1;
    require(demoted == inline_max, "exact_uint_subtract_failed");
    require(!demoted.uses_dynamic_storage(), "exact_uint_subtract_did_not_demote");
    require(
        demoted.to_u64() == std::numeric_limits<std::uint64_t>::max(),
        "exact_uint_checked_conversion_failed"
    );

    ExactUInt two_to_192 = 1;
    two_to_192 <<= 192;
    require(two_to_192.uses_dynamic_storage(), "exact_uint_wide_shift_did_not_spill");
    require(two_to_192.bit_length() == 193, "exact_uint_wide_bit_length_failed");
    require(
        two_to_192.to_decimal()
            == "6277101735386680763835789423207666416102355444464034512896",
        "exact_uint_wide_decimal_failed"
    );

    ExactUInt factor = two_to_64;
    factor += 1;
    const ExactUInt square = factor * factor;
    ExactUInt expected_square = 1;
    expected_square <<= 128;
    ExactUInt cross_term = 1;
    cross_term <<= 65;
    expected_square += cross_term;
    expected_square += 1;
    require(square.uses_dynamic_storage(), "exact_uint_product_did_not_spill");
    require(square == expected_square, "exact_uint_product_failed");

    bool conversion_rejected = false;
    try {
        static_cast<void>(two_to_64.to_u64());
    } catch (const IntegrityError &error) {
        conversion_rejected =
            std::string_view(error.what()) == "exact_uint_does_not_fit_u64";
    }
    require(conversion_rejected, "exact_uint_overflow_conversion_accepted");

    bool underflow_rejected = false;
    try {
        ExactUInt underflow = 0;
        underflow -= 1;
    } catch (const IntegrityError &error) {
        underflow_rejected =
            std::string_view(error.what()) == "exact_uint_subtraction_underflow";
    }
    require(underflow_rejected, "exact_uint_underflow_accepted");

    bool excessive_shift_rejected = false;
    try {
        ExactUInt excessive = 1;
        excessive <<= std::numeric_limits<std::uint64_t>::max();
    } catch (const IntegrityError &error) {
        excessive_shift_rejected =
            std::string_view(error.what()) == "exact_uint_shift_too_large";
    }
    require(excessive_shift_rejected, "exact_uint_excessive_shift_accepted");
    require(
        exponent_distance(
            std::numeric_limits<long long>::max(),
            std::numeric_limits<long long>::min()
        ) == std::numeric_limits<std::uint64_t>::max(),
        "exact_uint_signed_span_failed"
    );
}
...
class BitStream {
  public:
    ...
    ExactUInt bits_big(std::size_t count) {
        ExactUInt value = 0;
        for (std::size_t index = 0; index < count; ++index) {
            value <<= 1;
            value += bit();
    ...
    ExactUInt bounded_big(const ExactUInt &bound) {
        if (bound.is_zero()) throw IntegrityError("bitstream_nonpositive_big_bound");
        const ExactUInt top = bound - 1;
        const std::size_t width = top.bit_length();
        while (true) {
            const ExactUInt value = bits_big(width);
            if (value < bound) return value;
        }
    }
...
WeightTable make_weight_table(const std::vector<long long> &exponents) {
    ...
    table.span = exponent_distance(table.maximum, table.minimum);
    table.weights.reserve(exponents.size());
    for (const long long exponent : exponents) {
        const std::uint64_t shift = exponent_distance(exponent, table.minimum);
        ExactUInt weight = 1;
        weight <<= shift;
    ...
    ExactUInt legacy_total_limit = 1;
    legacy_total_limit <<= 63;
...
std::size_t choose_from_draw(const WeightTable &table, const ExactUInt &draw) {
    if (draw >= table.total) throw IntegrityError("draw_out_of_range");
    ExactUInt cumulative = 0;
...
SampleResult legacy_sample(
    ...
    for (const long long exponent : exponents) {
        const std::uint64_t shift = exponent_distance(exponent, minimum);
        if (shift >= 63) {
    ...
    for (std::size_t index = 0; index < weights.size(); ++index) {
        ...
        if (draw < cumulative) {
            return {index, ExactUInt(draw), stream.bits_read() - before, "legacy-u64"};
        }
    }
...
SampleResult exact_sample(const WeightTable &table, BitStream &stream) {
    ...
    const std::uint64_t before = stream.bits_read();
    const ExactUInt draw = stream.bounded_big(table.total);
    return {
...
std::string decimal(const ExactUInt &value) {
...
void audit_table(const WeightTable &table, bool exhaust_small, AuditCounts &counts) {
    ...
    for (std::size_t i = 0; i < table.weights.size(); ++i) {
        ...
        for (std::size_t j = 0; j < table.weights.size(); ++j) {
            if (table.exponents[i] >= table.exponents[j]) {
                ExactUInt expected = table.weights[j];
                expected <<= exponent_distance(table.exponents[i], table.exponents[j]);
                require(table.weights[i] == expected, "exact_weight_ratio_failed");
            } else {
                ExactUInt expected = table.weights[i];
                expected <<= exponent_distance(table.exponents[j], table.exponents[i]);
    ...
    if (exhaust_small) {
        ...
        std::vector<ExactUInt> observed(table.weights.size());
        const std::uint64_t total = table.total.to_u64();
        for (unsigned long long raw = 0; raw < total; ++raw) {
            ++observed[choose_from_draw(table, ExactUInt(raw))];
...
void audit_small_envelope_table(
    ...
    for (std::uint64_t raw = 0; raw < old_total; ++raw) {
        ...
        require(
            choose_from_draw(table, ExactUInt(raw)) == old_choice,
...
std::vector<SpanRecord> run_span_kats(AuditCounts &counts) {
    ...
    constexpr std::array<Expected, 9> EXPECTED{{
        ...
        {63, "9223372036854775808,1,1", "9223372036854775810", "exact-uint", 0,
            "4610367715746135718", 128},
        {64, "18446744073709551616,1,1", "18446744073709551618", "exact-uint", 0,
            "18441470862984542872", 130},
        {72, "4722366482869645213696,1,1", "4722366482869645213698", "exact-uint", 0,
            "4485372269814102570263", 365},
        {128, "340282366920938463463374607431768211456,1,1",
            "340282366920938463463374607431768211458", "exact-uint", 0,
            "129104461514002519291654946113109549607", 258},
        {192, "6277101735386680763835789423207666416102355444464034512896,1,1",
            "6277101735386680763835789423207666416102355444464034512898", "exact-uint", 0,
...
void run_fixture(std::ostream &output) {
    ...
    require(sum_result.path == "exact-uint", "sum_overflow_did_not_use_exact_uint");
    require(
    ...
    require(width64_result.path == "exact-uint", "width64_did_not_use_exact_uint");
    require(
    ...
    for (const int L : {6, 8}) {
        ...
        BitStream stream(sequential_seed());
        SampleResult result = exact_sample(table, stream);
        require(result.path == "exact-uint", "two_plane_did_not_use_exact_uint");
        if (L == 6) {
            require(

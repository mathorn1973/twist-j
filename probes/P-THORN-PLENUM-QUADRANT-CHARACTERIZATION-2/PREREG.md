# P-THORN-PLENUM-QUADRANT-CHARACTERIZATION-2 preregistration
...
Public claim lock: issue #637, opened before this file is committed.

```text
branch:  probe/P-THORN-PLENUM-QUADRANT-CHARACTERIZATION-2
path:    probes/P-THORN-PLENUM-QUADRANT-CHARACTERIZATION-2/
owner:   A. M. Thorn
...
`P-THORN-PLENUM-QUADRANT-CHARACTERIZATION-1`, issue #633 and PR #636, is consumed and abandoned.

```text
Status: ABANDONED.
PR_HEAD:        735f8b0564c1d2b7be1af304d2d8721b0dfeca48
IMMUTABLE_PIN:  fa899272eac305c040ab51f1fd3815363ebce574
```

Its first formal gate invocation printed diagnostic `PASS` lines through
`G11 mounted_third_root_lift_guard`, then exited nonzero at
`G12 scope_nonselector_guard`, wrote a traceback to stderr, and emitted no
decision line. The accepted G12 needle ended with the two adjacent words
`preferred embedding`, while the frozen preregistration ended one physical
line with `preferred` and began the next with `embedding`. This is the exact
G12 contiguous-string/newline defect. It is a verifier-integrity failure, not
a scientific falsifier.

The predecessor has no `EXPECTED.txt`, no `RUN.md`, no completed formal
gate, and no scientific, negative, candidate, selector, Galois, mounted-lift,
or physical conclusion. Its identifier, issue, PR, head, and pin must not be
reused or treated as evidence.

The carrier, theorem, falsifiers, mathematical gates, and mounted ownership
below are unchanged. The sole repair is whitespace normalization before the
frozen prefix scope test in G12. The successor uses a fresh issue,
identifier, preregistration, verifier, and immutable pin.
...
```text
P-THORN-PLENUM-QUADRANT-CHARACTERIZATION-2

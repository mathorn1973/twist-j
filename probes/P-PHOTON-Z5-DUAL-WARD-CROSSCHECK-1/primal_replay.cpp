...
ReplayOptions replay_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i += 2) {
        ...
        values.emplace(argv[i], argv[i + 1]);
    }
    const std::array<std::string, 6> required{{
        "--L", "--seed", "--start", "--thermal", "--samples", "--between"
    }};
    for (const auto &key : required)
        if (!values.count(key)) throw std::runtime_error("missing option: " + key);
    if (values.size() != required.size()) throw std::runtime_error("unknown or duplicate option");
    ...
    if (options.L < 2 || options.L > 32) throw std::runtime_error("L outside 2..32");
    if (options.start != "cold" && options.start != "hot")
        throw std::runtime_error("start must be cold or hot");
    if (!options.thermal || !options.samples || !options.between)
        throw std::runtime_error("schedule fields must be positive");
    return options;
}

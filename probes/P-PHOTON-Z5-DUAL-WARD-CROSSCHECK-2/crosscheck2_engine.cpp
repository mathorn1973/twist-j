...
// kernel used by P-PHOTON-Z5-DUAL-WARD-CROSSCHECK-2.  The transition kernel,
// selector probabilities, RNG draw order, and SHA-256 counter-bitstream domain
// are byte-for-byte inherited from P-PHOTON-Z5-DUAL-MOBILITY-QUALIFICATION-1.
// Development is explicit and hard-limited to L<=4.  Formal L=6,8 execution
// additionally requires the public pin commit and receipt and matches one of
// eight frozen seed/start/schedule rows exactly.
//
...
class Torus4 {
  public:
    explicit Torus4(int extent) : L(extent), volume(extent * extent * extent * extent) {
        if (L != 3 && L != 4 && L != 6 && L != 8) {
            throw IntegrityError("extent_must_be_3_4_6_or_8");
        }
        n_links = volume * DIM;
...
constexpr std::array<FormalSpec, 8> FORMAL_SPECS{{
    {6, "cold",       1, "0xbc2def7bcee975913c3b3b3999e83ad3"},
    {6, "cold",       2, "0x1a7ab1ad0011b62c04dcf48da9be3403"},
    {6, "stratified", 1, "0x5f0f36673dd145755b9a49e703aef3d6"},
    {6, "stratified", 2, "0x2b19daecb5c523f30bee3be7c047eb40"},
    {8, "cold",       1, "0x46ba01f80aec780ff9cc8b7e876c700c"},
    {8, "cold",       2, "0x2e0ccaa683e5f39f1237f05193b299c4"},
    {8, "stratified", 1, "0xf8f631709b4b9ce34f8a658bef3e1d0a"},
    {8, "stratified", 2, "0xfcd563ecc8bf8179b96c20db2c388307"},
}};

constexpr std::string_view FORMAL_PROBE = "P-PHOTON-Z5-DUAL-WARD-CROSSCHECK-2";
constexpr std::string_view SEED_ANCHOR = "072113d6a22fccef6468d2d647d006c262b6bf2d";

std::string formal_seed_preimage(const FormalSpec &spec) {
    return std::string(FORMAL_PROBE) + "|" + std::string(SEED_ANCHOR)
        + "|L=" + std::to_string(spec.L)
        + "|start=" + std::string(spec.start)
        + "|replica=" + std::to_string(spec.replica);
}

constexpr DevelopmentBudget FORMAL_BUDGET_L6{
    98'304,
    2'048,
    1'536,
    1'536,
...
constexpr DevelopmentBudget FORMAL_BUDGET_L8{
    262'144,
    2'048,
    4'096,
    4'096,
...
const DevelopmentBudget &formal_budget(int L) {
    if (L == 6) return FORMAL_BUDGET_L6;
    if (L == 8) return FORMAL_BUDGET_L8;
    throw IntegrityError("formal_budget_requires_L6_or_L8");
}
...
bool is_formal_seed_value(const Seed128 &seed) {
    for (const auto &spec : FORMAL_SPECS) {
        if (parse_seed(spec.seed_token).bytes == seed.bytes) return true;
    }
    return false;
}
...
std::vector<std::uint8_t> pack_state_2bit(const std::vector<std::uint8_t> &state) {
    std::vector<std::uint8_t> packed((state.size() + 3) / 4, 0);
    for (std::size_t index = 0; index < state.size(); ++index) {
        ...
        if (state[index] == 0) {
            ...
            throw IntegrityError("state_2bit_forbidden_residue");
        }
        const unsigned shift = static_cast<unsigned>(6 - 2 * (index % 4));
        packed[index / 4] |= static_cast<std::uint8_t>(code << shift);
    }
    return packed;
}
...
std::string base64_standard(const std::vector<std::uint8_t> &data) {
    ...
    for (std::size_t offset = 0; offset < data.size(); offset += 3) {
        ...
        const std::uint32_t second = remaining > 1 ? data[offset + 1] : 0;
        const std::uint32_t third = remaining > 2 ? data[offset + 2] : 0;
...
std::string state_2bit_base64(const Replica &replica) {
    return base64_standard(pack_state_2bit(replica.state));
}
...
class SectorEngine {
  ...
  private:
    ...
    void handle_bottom_checkpoint(std::ostream &output, bool accepted) {
        ...
        if (current_nonzero) checkpoint_nonzero_current_hashes_.insert(current_hash(target));
        checkpoint_state_hashes_.insert(state_hash(target));
    ...
    void emit_run(std::ostream &output) const {
        output
            ...
            << ",\"state_packed_bytes\":" << (lattice_.n_plaq + 3) / 4
            << ",\"state_unpacked_bytes\":" << lattice_.n_plaq
            << ",\"thin\":" << budget_.thin
    ...
    void emit_checkpoint(std::ostream &output, bool accepted) const {
        ...
        const std::vector<std::uint8_t> packed_state = pack_state_2bit(target.state);
        output
            << "{\"L\":" << lattice_.L
            ...
            << ",\"packed_state_sha256\":" << json_string(sha256(packed_state))
            << ",\"post_warm_bottom_attempt\":" << post_warm_bottom_attempts_
            << ",\"state_2bit_base64\":" << json_string(base64_standard(packed_state))
    ...
    void emit_summary(std::ostream &output) const {
        ...
        output
            ...
            << ",\"distinct_checkpoint_nonzero_current_hashes\":" << checkpoint_nonzero_current_hashes_.size()
            ...
            << ",\"checkpoint_nonzero_current_hashes\":" << string_set_json(checkpoint_nonzero_current_hashes_)
            ...
            << ",\"checkpoint_state_hashes\":" << checkpoint_state_hashes_.size()
            << ",\"thin\":" << budget_.thin
    ...
    std::set<std::string> checkpoint_nonzero_current_hashes_;
    std::set<int> nonzero_current_walkers_;
    std::set<std::string> checkpoint_state_hashes_;
};
...
void selftest_formal_firewall() {
    std::set<std::string> tuples;
    std::set<std::string> tokens;
    for (const auto &spec : FORMAL_SPECS) {
        const Seed128 parsed = parse_seed(spec.seed_token);
        require(
            sha256(formal_seed_preimage(spec)).substr(0, 32) == spec.seed_token.substr(2),
            "formal_seed_preimage_digest_mismatch"
        );
        require(
            formal_spec_matches(spec.L, spec.start, spec.seed_token, parsed),
            "formal_spec_rejected"
        );
        require(
            is_formal_seed_value(parsed),
            "formal_seed_firewall_rejected"
        );
        require(
        ...
        require(
            tuples.insert(
                std::to_string(spec.L) + ":" + std::string(spec.start) + ":"
                + std::to_string(spec.replica) + ":" + std::string(spec.seed_token)
            ).second,
        ...
        for (std::size_t index = 2; index < uppercase_token.size(); ++index) {
            if (uppercase_token[index] >= 'a' && uppercase_token[index] <= 'f') {
                uppercase_token[index] = static_cast<char>(
                    uppercase_token[index] - 'a' + 'A'
                );
                uppercased = true;
                break;
            }
        }
        require(uppercased, "formal_seed_has_no_uppercase_alias");
        require(
            parse_seed(uppercase_token).bytes == parsed.bytes,
            "uppercase_formal_seed_alias_changed_value"
        );
        require(
            !formal_spec_matches(spec.L, spec.start, uppercase_token, parsed),
            "noncanonical_formal_seed_accepted"
        );
        require(
            !formal_spec_matches(spec.L == 6 ? 8 : 6, spec.start, spec.seed_token, parsed),
            "wrong_formal_L_accepted"
        );
        const std::string_view other_start = spec.start == "cold" ? "stratified" : "cold";
        require(
            !formal_spec_matches(spec.L, other_start, spec.seed_token, parsed),
            "wrong_formal_start_accepted"
        );
    }
    require(tuples.size() == 8 && tokens.size() == 8, "formal_spec_count_failed");
    for (int L : {6, 8}) {
        const DevelopmentBudget expected = formal_budget(L);
        require(same_budget(expected, formal_budget(L)), "formal_budget_rejected");
        require(expected.validation_stride == expected.thin, "formal_validation_stride_not_thin");

        DevelopmentBudget changed = expected;
        ++changed.warm_bottom;
        require(!same_budget(changed, expected), "wrong_formal_warm_accepted");
        changed = expected;
        ++changed.checkpoints;
        require(!same_budget(changed, expected), "wrong_formal_checkpoints_accepted");
        changed = expected;
        ++changed.thin;
        require(!same_budget(changed, expected), "wrong_formal_thin_accepted");
        changed = expected;
        ++changed.validation_stride;
        require(!same_budget(changed, expected), "wrong_formal_validation_stride_accepted");
        changed = expected;
        ++changed.transition_cap;
        require(!same_budget(changed, expected), "wrong_formal_transition_cap_accepted");
    }
...
void run_selftest(std::ostream &output) {
    ...
    require(
        base64_standard(pack_state_2bit(std::vector<std::uint8_t>{0, 1, 4, 0})) == "GA==",
        "state_2bit_full_byte_KAT_failed"
    );
    require(
        base64_standard(pack_state_2bit(std::vector<std::uint8_t>{4, 1, 0, 4, 1})) == "kkA=",
        "state_2bit_tail_KAT_failed"
    );
    bool forbidden_residue_rejected = false;
    try {
        static_cast<void>(pack_state_2bit(std::vector<std::uint8_t>{3}));
    } catch (const IntegrityError &) {
        forbidden_residue_rejected = true;
    }
    require(forbidden_residue_rejected, "state_2bit_forbidden_residue_accepted");
    Seed128 seed;
...
void usage(std::ostream &output) {
    output
        << "usage:\n"
        << "  crosscheck2_engine --selftest\n"
        << "  crosscheck2_engine --development --L {3|4} --seed HEX128"
        << " --start {cold|stratified} --warm-bottom N --checkpoints N"
        << " --thin N --validation-stride N --transition-cap N\n"
        << "  crosscheck2_engine --formal --pin-commit HEX40"
        << " --pin-receipt ISSUE_COMMENT_URL --L {6|8} --seed 0xLOWERHEX32"
        << " --start {cold|stratified} --warm-bottom N --checkpoints N"
        << " --thin N --validation-stride N --transition-cap N\n";
}
...
Options parse_options(int argc, char **argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--development") {
            ...
            if (options.formal) throw IntegrityError("duplicate_formal_flag");
            options.formal = true;
...
int program_main(int argc, char **argv) {
    ...
    if (options.selftest) {
        if (options.development || options.formal || options.have_L || options.have_seed || options.have_start
    ...
    if (options.development == options.formal) {
        throw IntegrityError("select_exactly_one_of_development_or_formal");
    }
    ...
    if (options.development && is_formal_seed_value(options.seed)) {
        throw IntegrityError("development_forbids_formal_seed");
    }
    if (options.development && options.L != 3 && options.L != 4) {
        throw IntegrityError("development_L_must_be_3_or_4");
    }
    if (options.formal && options.L != 6 && options.L != 8) {
        throw IntegrityError("formal_L_must_be_6_or_8");
    }
    ...
    if (options.formal) {
        if (!(options.have_pin_commit && options.have_pin_receipt)) {
            throw IntegrityError("formal_requires_pin_commit_and_receipt");
        }
        ...
        if (!formal_spec_matches(options.L, options.start, options.seed_token, options.seed)) {
            throw IntegrityError("formal_spec_not_frozen");
        }
        if (!same_budget(options.budget, formal_budget(options.L))) {
            throw IntegrityError("formal_schedule_not_frozen");
        }
    }
...
int main(int argc, char **argv) {
    ...
    try {
        return crosscheck2::program_main(argc, argv);

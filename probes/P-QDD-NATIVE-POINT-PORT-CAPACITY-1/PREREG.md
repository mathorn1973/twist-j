# P-QDD-NATIVE-POINT-PORT-CAPACITY-1: preregistration

**PROSPECTIVE, RESULT-EXPOSED, L1 MATHEMATICAL COMPARISON ONLY.**

Identifier: `P-QDD-NATIVE-POINT-PORT-CAPACITY-1`.
Proposed claim: `QDD-NATIVE-POINT-PORT-CAPACITY`. Action layer: **L1**.
Owner: **A. M. Thorn / Codex-native-intake-20260919**.
Public reservation: [issue #1042](https://github.com/mathorn1973/twist-j/issues/1042).
Branch: `probe/P-QDD-NATIVE-POINT-PORT-CAPACITY-1`.
The current `POLICY.md` and `AGENTS.md` control public execution.
At preregistration, no formal gate has executed and no new status is earned.
...
the adjacent `verify.py`, with attribution retained.
...
The accepted verifier is the adjacent `verify.py`:
...
writes. Its companion `BREAKER.py` is a separately authored check from this
preregistration alone, with its own `BREAKER-PREREG.md`; the author-code
author and breaker author share exposed target statements, not implementation.
`verify.py` invokes `BREAKER.check()` and includes its deterministic summary,
so both required architecture jobs replay the independently written finite
check against the same exact expected stdout. All companion files are pinned
together before the first execution. Independence means implementation
separation, not blind prediction or complete independent physical confirmation. It audits the original generators against the quotient, selected
...
Before execution, freeze these accepted files on the reserved branch and
commit/push the preregistration, proof, source custody, verifier and independent
breaker/contract together. Record the pin and every file hash, then publicly
read the pin back and require exact byte identity and a clean checkout.
Only afterward run `python3 probes/P-QDD-NATIVE-POINT-PORT-CAPACITY-1/verify.py`
from the repository root in Linux, using the current repository runner's
600-second ceiling. Preserve stdout and stderr unchanged; record the actual
environment, return code, byte counts and hashes. Preserve a failed execution
under the repository disposition rules; do not adjust the pinned source or
thresholds. Complete exact-output and required two-architecture checks.
Independent proof review is separate from reproduction. This preregistration
does not itself supply a RUN, EXPECTED, result, Canon promotion or release.

...
Status: the post-pin aarch64 owner leg is complete and passes. The
two-architecture gate remains open pending the GitHub x86_64 run from the
same immutable pin.
...
GitHub   x86_64         pending   PENDING          same pinned verifier and transcript required
```
...
Two-architecture replay gate: **PENDING** until the GitHub x86_64 leg
reproduces the same verifier hash and stdout bytes. No claim, evidence row,
Canon file, dependency, or status changes in this probe.

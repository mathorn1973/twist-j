...
projectors E_low = (1/4) 1 1^T and E_high = I - E_low).  Rational arithmetic
only; no input, randomness, files, environment or network.
...
diagnostic, the value table of the two-outcome measure and the rational versus
mod-5 zero-sum counts.

Evidence for registry claims QDD-ALGEBRAIC-FACTORIZATION,
QDD-PROJECTOR-PAIR-TR4, QDD-QCARRIER-DIAGONAL-BOUNDARY,
QDD-BORN-READOUT-MEASURE and for the QUADRATIC-DECODER-DATA dictionary after
its fold.  The verifier confirms an already derived identity of the adopted
definitions; it is a conformance certificate, not an independent readout, not
a physical selection, and it decides no apparatus, occurrence law, sampling,
post-state or SI claim.  The value 1/6 in the table is a numerical witness with
no dependency.

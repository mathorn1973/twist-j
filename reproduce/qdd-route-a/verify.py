...
diagonal boundary of the two Q slots, and the value table of the
normalized pair.  Rational arithmetic only (fractions).  No input, no randomness, no
...
and QDD-QCARRIER-DIAGONAL-BOUNDARY.  QUADRATIC-DECODER-DATA remains an open
obligation; no completion-contract field is filled by this bundle.  The
verifier confirms an already derived identity; it is not an independent readout.
...
def R_cyc(w, low_line=LAMBDA_B):              # direct cyclotomic write; low_line varied only in a control
    ...
    return ("SUPPORTED", m, (w_low, w_high), ("DENSITY", dens), ("NORMALIZED", (w_low / m, w_high / m)))
...
# 14 value table of p_low over the 312 nonzero classes (numerical witness only)
hist = {}; classes = set()
for v in product(range(-2, 3), repeat=4):

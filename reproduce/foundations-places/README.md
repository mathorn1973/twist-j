...
Verified: the autonomous counter coordinate on the nonnegative forward
orbit of the 2-adic odometer. Its drive bit theta_n is the finite binary
digit-sum parity, with the exact recursions theta_2n = theta_n and
theta_(2n+1) = 1 - theta_n and the carry parity law, checked
exhaustively below 2^13 and matched over a long window by the local
carry rule alone. No parity function on all of Z_2 is asserted. Also
verified: the self similar time quantum tower on Z/5^k for k = 1 to 4
(M_J^(5^k) = i_5 I, the scalar of exact order 4
...
it; this witness computes k = 1 to 4 only and makes no all-k time
quantum claim. Check map: 01

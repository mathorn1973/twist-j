#!/usr/bin/env python3
"""Exact verifier for C-RECORD-CRT-IDEMPOTENT-1 rev2 (NON-CANONICAL note).

Scope: exactly the record-quotient calculus R1-R6 at layer L1. The three
arithmetic positions of J, the neighbouring-ring census, and cyclotomic
unit-rank minimality are NOT verified here; they belong to the companion
synthesis note as non-probe material.
...
Design rule, adopted after three defective rounds of this file: every gate
compares two routes computed by DIFFERENT code paths, and every gate is
covered by a mutation in mutation_test.py that must make it FAIL. A gate no
mutation can kill does not ship. The mutation harness is the guarantee. No
source scan for literal constants is attempted, because such a scan is
blind to construction-true conditions, which is how the previous rounds
failed.
...
def gate(label, condition, detail=""):
    ...
    line = "%-56s %s" % (label, "OK" if condition else "FAIL")
    if detail:
        line += "   " + detail
    print(line)
...
print("C-RECORD-CRT-IDEMPOTENT-1 rev2 -- record quotient calculus R1-R6")
...
print("Guarantee against tautological gates: mutation_test.py, not a source")
print("scan. Every gate below is killed by a mutation there.")
print("")
...
gate("W4  the norm is multiplicative: N(AB) = N(A) N(B)",
...
gate("F4  |kappa(P)| = N(P): the image of the reduction is the whole field",
     len(set(P2[0].reduce(v) for v in product(range(2), repeat=4))) == 16
     and len(set(P5[0].reduce(v) for v in product(range(5), repeat=4))) == 5
     and all(inorm(P.ideal) == P.norm for P in P2 + P5 + P3 + P11),
     "image counted, and N(P) taken as a lattice determinant")
_ringmap = True
...
# move both sides together; mutation M18 exists to enforce that.
...
gate("H3  the unique unital map is reduction on every basis vector",
     all(all(ired(rmul(hom_un[(a, b)][0], v), DATA[b][0]) == ired(v, DATA[b][0])
             for v in BASIS)
         for a in HOM for b in HOM if isub(DATA[a][0], DATA[b][0])),
     "f(z^j) = z^j mod I2 for j = 0..3, on the whole Z-basis")
...
gate("H6  the multiplicative maps are exactly the idempotents killed by I1",
     all(sorted(hom_all[(a, b)]) ==
         sorted(t for t in idempotents(DATA[b][0])
                if all(ired(rmul(g, t), DATA[b][0]) == ZERO
                       for g in DATA[a][0]))
         for a in HOM for b in HOM),
     "ties R4 back to R1: Hom is an idempotent computation")
...
gate("N1  every I_L has the SAME support and the SAME radical",
...
gate("N2  every I_L has the SAME Boolean algebra, |Idem| = 4",
...
gate("N3  L(R/I_L) = L is UNBOUNDED over the family",
     _lens == [1, 2, 3, 4, 5],
     "lengths %s from the lattice route" % (_lens,))
gate("N4  NO-GO: support and reduced record constant, filtration length not",
     len(set(NG[L][2] for L in range(1, 6))) == 1
     and len(set(len(NG[L][3]) for L in range(1, 6))) == 1
     and len(set(_lens)) == 5,
     "the Boolean skeleton cannot determine the completion depth")

print("")
_bad = [g for g, ok in GATES if not ok]
print("GATE COUNT: %d" % len(GATES))
if _bad:
    print("RESULT: FAIL (%d)" % len(_bad))
    for g in _bad:
        print("  failed: %s" % g)
    raise SystemExit(1)
print("RESULT: ALL %d GATES OK" % len(GATES))
raise SystemExit(0)

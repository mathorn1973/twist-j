#!/usr/bin/env python3
"""Job list and runner for C-PHOTON-PHASE-ORIENTATION-N v3 (NON-CANONICAL).
v3 = v2 with odd-N controls (Z_13 in place of Z_12); TWIST job list unchanged.

usage: jobs_v3.py list  MODELSET
       jobs_v3.py run   MODELSET OUTDIR WORKERS [--owner-authorized]
MODELSET: A3 (controls C1..C5) or B3 (TWIST point, needs --owner-authorized).
...
PHI = (1 + 5 ** 0.5) / 2
MODELS = {
    "C1": dict(N=13, beta=1.5, label="Z13 Wilson beta 3/2, Coulomb control"),
    "C2": dict(N=13, beta=0.7, label="Z13 Wilson beta 7/10, confined control"),
    "C3": dict(N=5, beta=5.0, label="Z5 Wilson beta 5, Higgs/frozen control"),
    "C4": dict(N=13, beta=1.1, label="Z13 Wilson beta 11/10, near-edge Coulomb control"),
    "C5": dict(N=13, beta=0.9, label="Z13 Wilson beta 9/10, near-edge confined control"),
    "TW": dict(N=5, beta=None, label="TWIST weight W at t = 1"),
}
SETS = {"A3": ["C1", "C2", "C3", "C4", "C5"], "B3": ["TW"]}
LS = [8, 12, 16, 24]
NMEAS, EVERY = 4000, 2
...
def main():
    ...
    if modelset == "B3" and mode == "run" and "--owner-authorized" not in sys.argv:
        sys.exit("refused: B3 needs --owner-authorized")
    js = jobs(modelset)
    if mode == "list":

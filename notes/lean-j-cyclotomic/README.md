# Lean note: the TWIST-J quartic

Status: **NON-CANONICAL NOTE**.

This directory is an exploratory manual artifact under `notes/`. It does not
...
The note formalizes one algebraic implication. In an arbitrary commutative
ring, assume that `ζ` satisfies the fifth cyclotomic relation
...
This note has no repository workflow and no automatic gate. Repository CI does
not build this Lean project. A successful manual build is an optional review
aid only and creates no scientific or procedural status. Admitting Lean
artifacts as public audit evidence requires a separate policy change with
explicit scope, provenance, dependency, and failure rules.

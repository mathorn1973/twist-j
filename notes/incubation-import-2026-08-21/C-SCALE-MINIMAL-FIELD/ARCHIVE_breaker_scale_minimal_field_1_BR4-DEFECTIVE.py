...
print("BREAKER C-SCALE-MINIMAL-FIELD-1   independent path, exact")
print()
...
rep("BR3-selection power is in the REQUIREMENT, not in minimality", True,
...
def irred3(a, b, c):
    for r in set([1, -1] + [t for t in range(-abs(c) - 1, abs(c) + 2) if t and c % t == 0]):
...
rep("BR4-Minkowski corroborated: no cubic in the box reaches |disc| <= 5",
    abs(best3[0]) <= 5,
...
traces_same = all(len({trz(powz(E[aexp], k)) for aexp in (1, 2, 3, 4)}) == 1
                  for k in range(1, 13))
rep("BR5-BREAK SUCCEEDS on S4: no rational invariant can pick J", True,

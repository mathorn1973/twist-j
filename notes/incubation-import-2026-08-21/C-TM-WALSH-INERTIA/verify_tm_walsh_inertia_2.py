...
def v6_witnesses():
    ...
    for tag, n, ps, cexp in wl:
        ...
        p_, q_, z_ = inertia(W)
        print("WITNESS %s n=%d k=%d c=%d inertia POS=%d NEG=%d ZERO=%d" % (
            tag, n, k, c1, p_, q_, z_))
        if cexp is not None:
            nz = [u for u in range(M) if W[u] != 0]
...
def main():
    N_CENSUS = 200000
    N_SMALL = 20000
    print("C-TM-WALSH-INERTIA-1 verifier v2 (v1 plus explicit inertia order; no gate changed)")
    print("INERTIA_ORDER POS NEG ZERO")
    print("every inertia tuple in this stdout reads (POS, NEG, ZERO); the Linux x86_64 leg")
    print("sigma=1 Hankel branch prints (NEG, ZERO, POS); never read one as the other")

...
def check_connected_edge_simple(jedges):
    ...
    if not is_ternary(jedges):
        raise ValueError("current is not ternary")
    if edge_d(jedges):
        raise ValueError("current is not closed")
    out = {}
    for (v, d), c in jedges.items():
        w = addv(v, unit4(d))
    ...
    while stack:
        ...
        for w in und[v]:
            if w not in seen:
                ...
                stack.append(w)
    if len(seen) != len(und):
        raise ValueError("current support is disconnected")
    ...
    circuit.reverse()
    if len(circuit) != len(jedges):
        raise ValueError("Eulerian walk misses support edges")
    if len(set(circuit)) != len(circuit):
        raise ValueError("Eulerian walk reuses a support edge")

    first_v, first_d = circuit[0]
    first_w = addv(first_v, unit4(first_d))
    walk_start = first_v if jedges[circuit[0]] > 0 else first_w
    cursor = walk_start
    for edge in circuit:
        v, d = edge
        w = addv(v, unit4(d))
        tail, head = (v, w) if jedges[edge] > 0 else (w, v)
        if tail != cursor:
            raise ValueError("Eulerian walk is not contiguous")
        cursor = head
    if cursor != walk_start:
        raise ValueError("Eulerian walk is not closed")

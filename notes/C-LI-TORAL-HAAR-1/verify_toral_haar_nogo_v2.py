#!/usr/bin/env python3
# verify_toral_haar_nogo_v2.py
# C-LI-TORAL-HAAR-1: PREP successor of verify_toral_haar_nogo.py (sha256
# da3bee431cfe807e62d03156f1d443735cc749d22245c04f3e01ab54658ec406) per
# P-J-LI-TORAL-HAAR-1_PREREG_DRAFT.md section 2. NON-FORMAL preparation:
# the formal probe re-pins a fresh copy at registration (owner rule; no
# retroactive formalization).
#
# Differences against v1, all required by the draft:
...
#     all-k, all-d statement is proof-first (lambda^k = 1 would make an
#     eigenvalue a root of unity, excluded by TH6).
#
...
#        rational root (p(1) = 1, p(-1) = 11; a rational root divides 1)
#        and no integer quadratic split (exact two-case enumeration);
#        hence p is irreducible over Q. {m <= 32 : eulerphi(m) = 4} =
#        {5, 8, 10, 12} by exact enumeration (the elementary bound
#        eulerphi(m) >= sqrt(m/2), imported, confines phi(m) = 4 to
#        m <= 32), and p differs coefficientwise from Phi_5, Phi_8,
#        Phi_10, Phi_12: no eigenvalue of M_J is a root of unity.
...
#        endpoints printed as directed 24-digit decimals.
#   X1   EXPLORATORY WITNESS (finite, exact, not a gate): det(A^k - I)
#        != 0 for the cat map [[2,1],[1,1]] with det(A^k - I) =
#        2 - tr(A^k), k = 1..300, and for M_J, k = 1..120. The all-k,
#        all-d statement follows from TH6, not from these loops.
#
...
# so it is +-1; p(1) = 1 != 0, p(-1) = 11 != 0 (TH4). No quadratic split:
# p = (x^2+ax+b)(x^2+cx+d) over Z forces bd = 1.
ok = True
for b, d in ((1, 1), (-1, -1)):
    found = False
    for a in range(-8, 9):
...
check("TH6 cyclotomic exclusion proof-first: p has no rational root and no integer quadratic split (exact enumeration), hence p is irreducible; {m : eulerphi(m) = 4} = {5,8,10,12} on the confining range m <= 32 (elementary bound eulerphi(m) >= sqrt(m/2), imported); p differs from Phi_5, Phi_8, Phi_10, Phi_12: no eigenvalue of M_J is a root of unity", ok)
...
for k in range(1, 121):
    ...
    ok = ok and detZ(QI) != 0
check("X1 EXPLORATORY WITNESS (finite, exact, not a gate): det(A^k - I) = 2 - tr(A^k) != 0 for the cat map, k = 1..300, and det(M_J^k - I) != 0 for k = 1..120; the all-k, all-d statement is TH5/TH6 proof-first", ok)

print("FAILED: " + (", ".join(FAILED) if FAILED else "none"))
print("VERDICT: " + ("PASS C-LI-TORAL-HAAR-1 prep successor v2" if not FAILED else "FAIL"))

...
- KERNEL-Z6-SYNCHRONIZATION [O]: the exact L1 decision for the declared autonomous update U on X = F_5^6: with X_z = {psi in X : z_6(psi) = z}, X_14 = X_1 union X_4, E_n(psi_0) = pr_checkpoint(U^n(0,psi_0)), and the sheet label q_n = 4 + 2 theta_(n-1) mod 5 for n >= 1, decide whether (i) for every n >= 3 and z in F_5 the restriction E_n|X_z is a bijection X_z -> X_(q_n), so at each fixed known n E_n:X->X_(q_n) is exactly 5-to-1 with one preimage in each initial sheet; (ii) for every n >= 1 the restrictions E_n|X_1 and E_n|X_4 are separately bijections onto X_(q_n), so E_n|X_14 is exactly 2-to-1 at that fixed n; (iii) for every seed both the checkpoint trace z_6(E_n(psi_0)) and checkpoint trajectory E_n(psi_0) are not eventually periodic; and (iv) for every seed no finite set S, self-map H:S->S, projection pi:S->X, and s_0 in S satisfy pi(H^n(s_0)) = E_n(psi_0) for all n >= 0; q_n is not the checkpoint coordinate q, and no unindexed checkpoint-fiber, census-recurrence, decoded-log, physical-irreversibility, unique infinite-realization, or L2-L6 claim is included
  Queue: ROOT; READY; FORMAL.
  Decision: closes positively only when a self-contained all-n proof establishes every universal clause and a complete exact audit of the frozen finite generator and sheet carrier passes the pinned aarch64/x86_64 byte-identity gate; closes negatively on one exact counterexample to any universal clause, including an exact eventual-period certificate for clause (iii) or an exact finite realization satisfying clause (iv); STOP on a broken declared premise, unresolved index or scope, incomplete proof or audit, invalid pin, authority mismatch, run-integrity failure, or transcript mismatch; a proof defect without an exact counterexample is STOP, not F

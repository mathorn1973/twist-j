...
exterior J predictive and semilinear closure, its Lorentzian factors,
minimal rational containing carrier, ramified trace-kernel bracket and
six-axis atlas; finite-reader and complete native-linear-word obstructions;
the typed classical digit-plus-carry exponent comparison; and complete
thirteen-class separable versus 3125-label reachable amplitude descriptions,
their selector common-factor boundary and conditional additive obstruction.

The fixed-axis 3-to-4 minimum is F-linear; the exact rational-source triple
already has a semilinear update. The 4-to-6 minimum is for Q-defined
containing carriers. Arbitrary target operators and amplitudes remain inputs.
No physical time, spatial dimension, apparatus, occurrence or layer bridge
is selected. All 422 prior Registry rows and all 28 live H/O obligations
retain their exact scopes and statuses; gates, Frontier programs and CORE
selections are unchanged. The release audit reconstructs exact v89 inputs
before every historical guard.
...
QUARTIC-CYCLOTOMIC-TOTAL-RAMIFICATION-CENSUS [T] registers the frozen L1
classification of full cyclotomic fields of degree four, quotiented by field
equality. The indices are exactly `5, 8, 10, 12`, with
`Q(zeta_10)=Q(zeta_5)` and three distinct fields `K_5`, `K_8`, and `K_12`.
The complete total-ramification locus is `{(K_5,5),(K_8,2)}`. Their residue
unit groups are `F_5^x=C_4`, generated by the inherited reduction of `J` to
2, and `F_2^x=C_1`; the ramified but non-total `K_12` controls remain
explicit as `F_4^x=C_3` above 2 and `F_9^x=C_8` above 3.

The status is T because the owner independently accepted the complete proof
in `PREREG.md` as theorem-grade at this exact scope. The pinned verifier
produced the same nine-gate transcript byte for byte on x86_64 and aarch64,
and the independently frozen blind breaker found no break in ten gates.
Computation alone would remain C. The row is not T-LOCK.

This is an L1 arithmetic-only fold. It selects neither degree four nor a
broader CM class, proves no unique physical field or place, does not strengthen
TWO-PLACE-PHYSICS [D], and supplies no decoder, Born measure, physical bit,
clock, force, observable, or L2-L6 lift. No live H or O row moves.


## Public Canon v32

<!-- BEGIN GENERATED CANON32 COUNTS -->
Registry snapshot: 218 claims; 0 T-LOCK, 116 T, 40 D, 24 C, 4 H, 24 O, 10 F; 28 live H/O.
Reproduction witnesses: 22.
<!-- END GENERATED CANON32 COUNTS -->
...
failure of one proposed bridge is not a negative theorem. Any exact
finite-cylindrical exhibition at a registered cursor and window from 4
through 32 at any finite lambda-depth would refute
ENTROPY-CYLINDER-NOGO-CURSOR rather than close `A_A`; inside
the older cut scope it would also require correction of that F row. The fold
also corrects the historical ENTROPY-CYLINDER-CUT coverage to the cases
actually established by its own public verifier.
...
Carry-pentad lane: `CARRY-PENTAD` enters at [T] and L1. In the frozen
four-coordinate Hamming frame, the second carry bit is the minus-type
quadratic refinement whose five nonzero singular vectors carry `S_5`; the
raw bit-intersection form has only `S_4 x C_2`. Four is the minimum binary
linear width for a fixed order-five target. The integral `A4` augmentation
lattice connects the five-cycle to `M_J`, its ramified quotient reads every
`I+C^a` as 2, and the mod-two transvection supplies the typed root-lattice
bridge. No unconditional prime, width, cycle, exponent, gauge, or physical
reading is selected.

Metal-trace lane: `METAL-TRACE-CASCADE` enters at [T] and L2, scoped to
`t in {1,2}`. The two laws `x^2=t x+1` give discriminants 5 and 8,
Fibonacci and Pell growth, golden unit `J`, and the nonunit silver pure form
`1+i=sqrt2 m_8` of norm 4 in `Q(zeta_8)`. The run/record reading remains
outside the theorem.

Census evidence: `CENSUS-313`, `CENSUS-Z5-SHEET`, `CENSUS-PAIRING`, and
`CENSUS-HOSTING` remain [C], but their unchanged frozen verifier now has a
byte-identical aarch64/x86_64 public replay. This is reproduction, not an
independent implementation and not protocol independence.

Evidence: `probes/P-RAMIFIED-TM-LIFT-1`, `probes/P-CARRY-PENTAD-1`,
`probes/P-METAL-TRACE-1`, and `probes/P-CENSUS-REPLAY-1`.

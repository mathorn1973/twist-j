...
The Born quartet [T]: the unit square root of a phase is the Born
normalized bisector; the Born faithful residuals are A_4 = Z[i]/5 and
A_8 = Z[zeta_8]/5 with the Born involution the swap; the quarter turn
spinor is R = (1 - B)/sqrt2 with det R = 1; each Born halving is one
quadratic step, and the staircase 4 -> 8 -> 16 is F_5 -> F_25 -> F_625,
norm gated.
...
dense in SO+(3,1); the boost rapidity is ln phi. Velocity is a count
[D]: the substrate counts integer rapidity steps ln phi; the decoder
reads beta_n = tanh(n ln phi) with exact Fibonacci and Lucas parity;
Einstein velocity composition is index addition; subluminality is the
unit norm identity L^2 - 5 F^2 = 4 (-1)^n in Z[phi]. The observer
alternator [D]: the observer reads 4 = 1 + 3, the substrate counts
4 = 2 + 2; the hinge between the two partitions is the alternator with
lambda = -1. The boost axis is the alternator [D]. The boost reading
split holds [T].
